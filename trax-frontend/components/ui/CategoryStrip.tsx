...
export const categories = [
  ...
  { label: 'Ecosystem',   color: 'var(--accent-bright)' },
          ...
          {displayCategories.map((cat) => {
            ...
            return (
              <motion.button
                ...
                whileTap={{ scale: 0.96 }}
                className="relative flex-shrink-0 px-4 py-2 rounded-full text-sm font-semibold transition-all duration-200 whitespace-nowrap hover:bg-[rgba(255,26,26,0.05)] hover:text-accent-bright"
                style={{
                  fontFamily:      'var(--font-dm-sans)',
                  backgroundColor: isActive ? 'var(--accent-bright)' : 'transparent',
                  color:           isActive ? '#fff' : 'var(--fg-muted)',
                  border:          `1.5px solid ${isActive ? 'var(--accent-bright)' : 'transparent'}`,
                  boxShadow:       isActive ? '0 4px 12px rgba(255, 26, 26, 0.15)' : 'none',
                }}
              >
                {cat.label}
              </motion.button>
